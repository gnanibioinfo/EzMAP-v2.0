...
# Ensure browseURL() has a usable browser on Linux/macOS. The conda R that EzMAP
# uses has no 'browser' option set by default, so launch.browser=TRUE would abort
# the whole app with "'browser' must be a non-empty character string". Set a sane
# opener here (runs before the server launches the browser).
local({
  if (!nzchar(getOption("browser", ""))) {
    os <- Sys.info()[["sysname"]]
    if (identical(os, "Linux"))  options(browser = "xdg-open")
    if (identical(os, "Darwin")) options(browser = "open")
  }
})
