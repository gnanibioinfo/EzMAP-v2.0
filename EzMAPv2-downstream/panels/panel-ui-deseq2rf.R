################################################################################
# panels/panel-ui-deseq2rf.R — DESeq2 + RF Combined Panel UI (unified layout)
################################################################################

library(shinycssloaders)

deseq2rfUI <- function(id, guide = NULL) {
    ns <- NS(id)
    ...
    controls <- tagList(
        ...
        conditionalPanel(
            ...
            tags$div(
                ...
                tags$p(
                    ...
                    strong("Easy Mode: "),
                    "Using default DESeq2 and Random Forest cutoffs. To customize, restart the app and select Expert mode."
                )
            )
        ),

        # ----- Conditions to compare (mirrors the DESeq2 tab) -----
        ...
        helpText(style = "font-size:11px; color:#64748B;",
                 "Pick the same two groups you used in the DESeq2 and ",
        ...
        # Live status pulled from upstream DESeq2 / RF runs
        ...
        conditionalPanel(
            ...
            ns = shiny::NS(NULL),
            tagList(
                numericInput(ns("padj_cutoff"),   "DESeq2 adjusted p-value cutoff:",
                             value = 0.05, min = 0, max = 1, step = 0.01),
                numericInput(ns("log2fc_cutoff"), "DESeq2 |log2 fold change| cutoff:",
    ...
    aesthetics <- tagList(
        ...
        hr(),
        helpText(
            "Run DESeq2 and Random Forest in their own tabs first. ",
    ...
    plot_area <- tagList(
        tabsetPanel(
            tabPanel("Overview",
                h5(icon("bullseye"), "How many ASVs overlap?"),
            ...
            tabPanel("Scatter: Log2FC vs. Gini",
                h5(icon("chart-area"), "DESeq2 effect size vs. RF importance"),
                p("Each dot is one ASV. Red dots meet both the DESeq2 significance cutoff and appear in the top-N of the RF importance ranking."),
                shinycssloaders::withSpinner(
                    plotOutput(ns("scatter_plot"), height = "600px"),
                    type = 6, color = "#3B82F6", size = 0.9,
                    caption = "Joining DESeq2 & RF results..."
                )
            )
        )
    )
    ...
    stats_area <- tagList(
        tabsetPanel(
            tabPanel("Intersection Table",
                h5(icon("table"), "ASVs significant in BOTH analyses"),
                shinycssloaders::withSpinner(
    ...
    tagList(
        h3("Combined DESeq2 + Random Forest ASV Selection"),
        p(style = "color:#64748B;",
          "This panel intersects the ASVs flagged by DESeq2 (differential abundance) ",
