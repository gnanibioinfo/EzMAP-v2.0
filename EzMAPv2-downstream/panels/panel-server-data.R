    ...
    local({
      ...
      physeq_object <- reactive({
        if (auto_loaded()) {
          ...
          # Manual upload path: BIOM + metadata are required; the phylogenetic
          # tree is OPTIONAL (only needed for UniFrac / phylogenetic diversity).
          # Previously this req() also demanded input$treeFile, so uploading
          # just BIOM + metadata left the phyloseq build blocked forever and the
          # data-summary / metadata spinners span indefinitely — the app looked
          # stuck on "loading" and no downstream tab could start.
          req(input$biomFile, input$metaFile)
          tree_path <- if (!is.null(input$treeFile)) input$treeFile$datapath else NULL
