...
tax4fun2Server <- function(id, physeq_raw, physeq_filtered, global_state_rv) {
  moduleServer(id, function(input, output, session) {
    ...
    tax4fun_results <- eventReactive(input$run_tax4fun2, {
      ...
      result <- tryCatch({
        ...
        withProgress(message = "Running Tax4Fun prediction...", value = 0, {
          ...
          incProgress(1, detail = "Done")
          if (is.null(fxn_table) || nrow(fxn_table) == 0 || ncol(fxn_table) == 0) {
            showNotification(
              paste0("Tax4Fun: no taxa matched the SILVA reference, so no functions ",
                     "were predicted. This is expected for non-SILVA or synthetic/example ",
                     "taxonomy; other analyses are unaffected."),
    ...
    output$tax4fun2_heatmap <- renderPlot({
      ...
      # Clear, specific message when Tax4Fun predicted nothing because no taxa
      # matched the SILVA reference (e.g. taxonomy not assigned with SILVA, or
      # synthetic/example data). Otherwise the user only sees a generic error.
      res0 <- tax4fun_results()
      if (!is.null(res0) && (is.null(res0$fxn_table) ||
            nrow(res0$fxn_table) == 0 || ncol(res0$fxn_table) == 0)) {
        return(
          ggplot2::ggplot() + ggplot2::theme_void() +
            ggplot2::annotate("text", x = 0.5, y = 0.62,
                     label = "No functions could be predicted",
                     size = 6, fontface = "bold", color = "#D97706") +
            ggplot2::annotate("text", x = 0.5, y = 0.44,
                     label = paste(
                       "None of the taxa matched the SILVA reference that Tax4Fun uses.",
                       "Tax4Fun requires 16S taxonomy assigned with SILVA; non-SILVA or",
                       "synthetic/example taxonomy will not map to KEGG functions.",
                       "Your other analyses (diversity, DESeq2, etc.) are unaffected.",
                       sep = "\n"),
                     size = 4, color = "#7F8C8D") +
            ggplot2::xlim(0, 1) + ggplot2::ylim(0, 1)
        )
