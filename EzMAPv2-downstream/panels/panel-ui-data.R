                ...
                bslib::card(class = "card-upload",
                    ...
                    if (is_auto) {
                        ...
                        tagList(
                            fileInput(ns("biomFile"), "Upload BIOM File (.biom)", accept = ".biom"),
                            fileInput(ns("metaFile"), "Upload Sample Metadata (.tsv)", accept = ".tsv"),
                            fileInput(ns("treeFile"), "Upload Phylogenetic Tree (.nwk) — optional", accept = c(".nwk", ".txt")),
                            hr(),
                            uiOutput(ns("renameConflictUI")),
                            helpText(style = "font-size:11px; color:#94A3B8;",
                                     tags$b("BIOM"), " and ", tags$b("metadata"),
                                     " are required; the tree is optional ",
                                     "(only needed for UniFrac / phylogenetic diversity). ",
                                     "Data loads as soon as both required files are added. Max 50 MB.")
                        )
                    }
                )
            ),
