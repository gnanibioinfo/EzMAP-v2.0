...
public class ManifestPage extends BasePage {
    ...
    public ManifestPage(WizardController wizard) {
        super("Expert Mode — Step 1: Validate Inputs",
              ...
              "EzMAP v2.0 will auto-detect the FASTQ format, generate a QIIME2 manifest, " +
        ...
        ampliconCard.row(new InfoBanner(InfoBanner.Kind.INFO, "About your data",
                "Tell EzMAP v2.0 about your amplicon region and whether primers/barcodes are still in the reads. " +
                "This information is used for primer removal, quality trimming, and overlap estimation. " +
                "If you don\u2019t know, select \u201CI don\u2019t know\u201D and EzMAP v2.0 will auto-detect."))
                .gap(10);
        ...
        actionCard.row(new InfoBanner(InfoBanner.Kind.INFO,
                ...
                "If IDs don\u2019t match exactly, EzMAP v2.0 uses intelligent fuzzy matching and asks for approval."))
               .gap(10);
    ...
    private File findScript(File baseDir) {
        ...
        // Next to the JAR first — robust no matter what the working directory is.
        try {
            String jarPath = ManifestPage.class.getProtectionDomain()
                    .getCodeSource().getLocation().getPath();
            File jarDir = new File(java.net.URLDecoder.decode(jarPath, "UTF-8")).getParentFile();
            if (jarDir != null) {
                candidates.add(new File(jarDir, "scripts" + File.separator + name));
                candidates.add(new File(jarDir, name));
                File p1 = jarDir.getParentFile();
                if (p1 != null) {
                    candidates.add(new File(p1, "scripts" + File.separator + name));
                    candidates.add(new File(p1, name));
                    File p2 = p1.getParentFile();
                    if (p2 != null) candidates.add(new File(p2, "scripts" + File.separator + name));
                }
            }
        } catch (Exception ignored) {}
        // Then relative to the provided base directory (working dir).
        candidates.add(new File(baseDir, name));
        candidates.add(new File(baseDir, "scripts" + File.separator + name));
        if (baseDir.getParentFile() != null) {
            candidates.add(new File(baseDir.getParentFile(), name));
            candidates.add(new File(baseDir.getParentFile(), "scripts" + File.separator + name));
        }
        ...
        new Thread(() -> {
            try {
                ...
                if (os.contains("win")) {
                    ...
                    // macOS / Linux: use the conda env's python3 (via a login shell),
                    // not the bare system python3 — on macOS /usr/bin/python3 is an
                    // Xcode Command Line Tools stub that fails when CLT aren't installed.
                    String cmd = condaPreamble()
                            + "python3 " + q(scriptFile.getAbsolutePath())
                            + " " + q(fastqDir) + " " + q(manifestPath)
                            + " --metadata " + q(metaFile);
        ...
        try {
            ...
            String explanation = allMatched
                ? "<html><body style='width:680px'>FASTQ filenames produce <b>different sample IDs</b> "
                  + "than your metadata. EzMAP v2.0 auto-matched them. <b>Review and click OK if correct.</b> "
            ...
            return JOptionPane.showConfirmDialog(
                    ...
                    "EzMAP v2.0", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION;
        }
    }
    ...
    // Single-quote for bash sent via wsl.exe — double quotes get mangled through
    // Windows -> wsl.exe -> bash and split paths at spaces (OneDrive, spaced
    // folder names). Single quotes survive; embedded ' escaped the bash-safe way.
