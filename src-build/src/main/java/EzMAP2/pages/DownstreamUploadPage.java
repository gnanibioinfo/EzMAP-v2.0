...
public class DownstreamUploadPage extends BasePage {
    ...
    static boolean ensureRPackages(LogConsole console, JLabel statusLabel) {
        ...
        try {
            ...
            if (isWindows) {
                ...
                // macOS / Linux: run Rscript from the conda EzMAP2-qiime2 env (the
                // QIIME2 distro ships R); a bare `Rscript` is usually NOT on PATH
                // unless conda has been initialised in the user's shell profile.
                showInstallBanner.run();
                pb = new ProcessBuilder("bash", "-lc",
                        condaInitPreamble() + "Rscript '" + installerScript.getAbsolutePath() + "'");
            }
    ...
    static ProcessBuilder buildShinyProcess(String osName, File appDir,
        ...
        // Open the browser via an OS-aware, error-safe helper. The default
        // launch.browser=TRUE calls utils::browseURL(), which HALTS the whole
        // server if R has no 'browser' option configured (as the conda R does not
        // on Linux/macOS), giving "'browser' must be a non-empty character string".
        // Wrapping it in tryCatch keeps Shiny running even if the browser can't be
        // opened — the user can then open the printed URL manually.
        rScript.append("ezmap_open <- function(url) {\n");
        rScript.append("  os <- Sys.info()[['sysname']]\n");
        rScript.append("  tryCatch({\n");
        rScript.append("    if (os == 'Darwin') system2('open', url, wait = FALSE)\n");
        rScript.append("    else if (os == 'Linux') system2('xdg-open', url, wait = FALSE)\n");
        rScript.append("    else utils::browseURL(url)\n");
        rScript.append("  }, error = function(e) message('Open this URL in your browser: ', url))\n");
        rScript.append("}\n");
        rScript.append("shiny::runApp('")
               .append(rPath(appDir.getAbsolutePath(), convertPaths))
               .append("', launch.browser = ezmap_open)\n");
        ...
        if (isWindows && useWsl) {
            ...
            // --- macOS / Linux ---
            // Run Rscript from the conda EzMAP2-qiime2 env (the QIIME2 distro ships
            // R). A bare `Rscript` is usually NOT on PATH unless conda is initialised
            // in the shell profile, which is why launches failed with "command not
            // found" on macOS.
            pb = new ProcessBuilder("bash", "-lc",
                    condaInitPreamble() + "Rscript '" + tempR.getAbsolutePath() + "'");
            pb.directory(appDir);
        }
