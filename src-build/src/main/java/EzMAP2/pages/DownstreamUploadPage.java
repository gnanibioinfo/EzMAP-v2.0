...
public class DownstreamUploadPage extends BasePage {
    ...
    static boolean ensureRPackages(LogConsole console, JLabel statusLabel) {
        ...
        SwingUtilities.invokeLater(() -> {
            console.info("First launch: installing the R packages required for the "
                    + "downstream analyses. This is a one-time setup and may take a few "
        ...
        try {
            ...
            if (isWindows) {
                ...
                if (nativeRscript != null) {
                    final String rsPath = nativeRscript;
                    SwingUtilities.invokeLater(() ->
                            console.info("Using R: " + rsPath));
                    pb = new ProcessBuilder(nativeRscript, installerScript.getAbsolutePath());
    ...
    static ProcessBuilder buildShinyProcess(String osName, File appDir,
        ...
        if (isWindows) {
            nativeRscript = resolveWindowsRscript();
            hasNativeR = nativeRscript != null;
            if (!hasNativeR) {
