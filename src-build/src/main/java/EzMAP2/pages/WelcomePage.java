...
public class WelcomePage extends BasePage {
    ...
    // The real content (cards + results + button) stays hidden behind a spinner
    // until the system check finishes.
    private JPanel content, loadingPanel;
    ...
    public WelcomePage(WizardController wizard) {
        ...
        // ---- Assemble the real content (hidden until the system check finishes) ----
        content = new JPanel();
        content.setOpaque(false);
        content.setLayout(new BoxLayout(content, BoxLayout.Y_AXIS));
        content.setAlignmentX(LEFT_ALIGNMENT);
        addToColumn(content, row);
        addToColumn(content, sysPanel);
        addToColumn(content, tip);
        addToColumn(content, navRow);

        // ---- Loading state: spinner shown while the system check runs ----
        loadingPanel = buildLoadingPanel();

        add(loadingPanel);
        add(content);
        content.setVisible(false);   // revealed by revealContent() when the check completes
    }

    /** Append to a vertical column the way BasePage.add does (alignment + width stretch). */
    private static void addToColumn(JPanel col, JComponent c) {
    ...
    /** Spinner + message shown on first launch while memory/disk are checked. */
    private JPanel buildLoadingPanel() {
        JPanel p = new JPanel();
        p.setOpaque(false);
        p.setLayout(new BoxLayout(p, BoxLayout.Y_AXIS));
        p.setAlignmentX(LEFT_ALIGNMENT);
        p.setBorder(new EmptyBorder(48, 4, 48, 4));

        JLabel heading = new JLabel("Checking your system…");
        heading.setFont(Theme.FONT_BODY_BOLD);
        heading.setForeground(Theme.INK_1);
        heading.setAlignmentX(LEFT_ALIGNMENT);

        JProgressBar spinner = new JProgressBar();
        spinner.setIndeterminate(true);
        spinner.setAlignmentX(LEFT_ALIGNMENT);
        spinner.setMaximumSize(new Dimension(300, 18));
        spinner.setPreferredSize(new Dimension(300, 18));

        JLabel sub = new JLabel("<html><body style='width:680px;color:#64748B'>"
                + "Measuring available memory and free disk space. This can take a few "
                + "seconds on first launch while WSL starts up — your options will "
                + "appear as soon as the check completes.</body></html>");
        sub.setFont(Theme.FONT_SMALL);
        sub.setForeground(Theme.INK_2);
    ...
    /** Hide the spinner and reveal the selection cards + Get Started button. */
