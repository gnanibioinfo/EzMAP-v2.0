...
public class EasyModePage extends BasePage {
        ...
        new Thread(() -> {
            try {
                ...
                if (osName.contains("win")) {
                    ...
                    // macOS / Linux: run through the conda env's python3, NOT the bare
                    // system python3. On macOS, /usr/bin/python3 is an Xcode Command
                    // Line Tools stub that fails with "No developer tools were found"
                    // when CLT aren't installed. Sourcing conda first gives a real
                    // python3 with all dependencies and avoids that stub entirely.
                    String cmd = condaPreamble()
                            + "python3 " + q(genScript) + " " + q(fastq)
                            + " " + q(manifestPath) + " --metadata " + q(meta);
                ...
                if (genExit != 0) {
                    ...
                    final String low = detail.toLowerCase();
                    final boolean win = System.getProperty("os.name").toLowerCase().contains("win");
                    SwingUtilities.invokeLater(() -> {
                        ...
                        if (low.contains("xcode") || low.contains("developer tools")) {
                            console.err("Tip (macOS): install Apple's Command Line Tools once with "
                            ...
                            console.err("Tip: if the path to your data or metadata contains spaces, move it "
                                    + "to a simple path (e.g. ~/EzMAP2_data) and retry.");
                        }
    ...
    private void runPipeline(String fastq, String meta, String out,
        ...
        try {
            String osName = System.getProperty("os.name").toLowerCase();
            String projectDir = System.getProperty("user.dir");
            // Resolve next to the JAR (not the cwd) so it works no matter how the
            // app was launched — a cwd-relative path fails on native Linux/macOS
            // with exit 127 ("No such file or directory").
            String scriptHost = findAsset("scripts/easy_mode.sh");
            if (scriptHost == null) {
                scriptHost = projectDir + File.separator + "scripts" + File.separator + "easy_mode.sh";
            }
            ...
            SwingUtilities.invokeLater(() -> {
                ...
                } else if (exit == 15 || exit == 137 || exit == 143 || exit == 9
                        || exit == 134 || exit == 139) {
                    console.err("Pipeline stopped (exit " + exit + ") — most likely OUT OF "
                            + "MEMORY, commonly at the taxonomy step where the classifier is "
                            + "loaded into RAM.");
                    console.err("Fixes: (1) keep Low-memory mode ON; (2) use a smaller "
                            + "classifier — Greengenes2 V4 instead of a full-length one; or "
                            + "(3) give WSL more memory: create %USERPROFILE%\\.wslconfig with "
                            + "[wsl2] then memory=16GB, run 'wsl --shutdown', and retry.");
                } else {
    ...
    private static String findGenerateManifest(String projectDir) {
        ...
        // Resolve next to the JAR first (works no matter what the cwd is).
        String byAsset = findAsset("scripts/" + name);
        if (byAsset == null) byAsset = findAsset(name);
    ...
    /**
     * Resolve a bundled asset (e.g. "scripts/easy_mode.sh") to an absolute host
     * path, searching next to the JAR first and then the working directory, so it
     * works no matter how the app was launched (the cwd may not be the project
     * root). Returns null if not found.
     */
    private static String findAsset(String relativeName) {
        java.util.List<File> tries = new java.util.ArrayList<>();
        try {
            String jarPath = EasyModePage.class.getProtectionDomain()
                    .getCodeSource().getLocation().getPath();
            File jarDir = new File(java.net.URLDecoder.decode(jarPath, "UTF-8")).getParentFile();
            if (jarDir != null) {
                tries.add(new File(jarDir, relativeName));
                File p1 = jarDir.getParentFile();
                if (p1 != null) {
                    tries.add(new File(p1, relativeName));
                    File p2 = p1.getParentFile();
                    if (p2 != null) tries.add(new File(p2, relativeName));
                }
            }
        } catch (Exception ignored) {}
        File cwd = new File(System.getProperty("user.dir"));
        tries.add(new File(cwd, relativeName));
        if (cwd.getParentFile() != null) tries.add(new File(cwd.getParentFile(), relativeName));
        for (File f : tries) {
    ...
    /** Sources conda and activates the QIIME2 env for macOS/Linux shell commands. */
    private static String condaPreamble() {
        return "for d in \"$HOME/miniconda3\" \"$HOME/miniforge3\" \"$HOME/mambaforge\" "
             + "\"$HOME/anaconda3\" \"/opt/miniconda3\" \"/opt/conda\"; do "
             + "if [ -f \"$d/etc/profile.d/conda.sh\" ]; then . \"$d/etc/profile.d/conda.sh\"; break; fi; "
             + "done; conda activate EzMAP2-qiime2 2>/dev/null; ";
    }
