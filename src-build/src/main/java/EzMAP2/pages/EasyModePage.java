...
public class EasyModePage extends BasePage {
    ...
    public EasyModePage(WizardController wizard) {
        super("Easy Mode — Guided Pipeline",
              ...
              "EzMAP v2 first validates your inputs and matches sample IDs, " +
        ...
        JLabel valHelp = new JLabel(
            ...
            + "If IDs don't match exactly, EzMAP v2 uses intelligent fuzzy matching and asks for your approval."
            + "</body></html>");
    ...
    private void runValidation() {
        ...
        new Thread(() -> {
            try {
                ...
                if (genExit != 0) {
                    SwingUtilities.invokeLater(() -> {
                        console.err("Validation failed. Check paths and ensure python3 is available in WSL.");
    ...
    private boolean showReconciliationDialog(String jsonStr, String fastqDir) {
        try {
            ...
            JDialog dlg = new JDialog(
                    (Frame) SwingUtilities.getWindowAncestor(this),
                    allMatched ? "Sample ID Reconciliation \u2014 EzMAP v2"
                               : "Sample ID Mismatch \u2014 EzMAP v2", true);
            dlg.setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);
            ...
            if (allMatched) {
                explanationText = "<html><body style='width:680px'>"
                    ...
                    + "EzMAP v2 auto-matched them using intelligent fuzzy matching."
                    + "<br><br>"
            ...
            int ch = JOptionPane.showConfirmDialog(
                    ...
                    "EzMAP v2", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
    ...
    private void runPipeline(String fastq, String meta, String out,
        ...
        try {
            ...
            if (osName.contains("win")) {
                ...
                pb = new ProcessBuilder("wsl.exe", "-d", "Ubuntu", "--", "bash", "-lc", cmd.toString());
            } else {
    ...
    private void showResults(String outDir) {
        ...
        html.append("<br><i>Load this bundle in the EzMAP v2 Downstream (Shiny) module.</i>");
