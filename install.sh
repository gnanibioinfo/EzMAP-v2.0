...
# ---- Detect OS + CPU architecture so we fetch the right installers ----
# A Linux Miniconda installer on macOS fails with "cannot execute binary file"
# (exit 126), so pick the build that matches this machine.
OS_KERNEL="$(uname -s)"
OS_ARCH="$(uname -m)"
CONDA_SUBDIR_OVERRIDE=""        # set on Apple Silicon (QIIME2 ships no native arm64 build)
case "$OS_KERNEL" in
    Darwin)
        QIIME2_OS="osx"
        if [ "$OS_ARCH" = "arm64" ]; then
            MINICONDA_URL="https://repo.anaconda.com/miniconda/Miniconda3-latest-MacOSX-arm64.sh"
            CONDA_SUBDIR_OVERRIDE="osx-64"   # QIIME2 = Intel build, runs under Rosetta 2
        else
...
QIIME2_YML_URL="https://data.qiime2.org/distro/amplicon/qiime2-amplicon-${QIIME2_VERSION}-py310-${QIIME2_OS}-conda.yml"
...
if [ "$SKIP_QIIME" = true ]; then
    ...
    if ask_yes "Install QIIME2 ${QIIME2_VERSION}? (Required for upstream analysis, ~5 GB)"; then
        ...
        if [ -n "$CONDA_SUBDIR_OVERRIDE" ]; then
            warn "Apple Silicon detected — installing the Intel (osx-64) QIIME2 build to run under Rosetta 2."
            if ! /usr/bin/arch -x86_64 /usr/bin/true 2>/dev/null; then
                warn "Rosetta 2 may be missing. If this fails, run: softwareupdate --install-rosetta --agree-to-license"
            fi
            CONDA_SUBDIR="$CONDA_SUBDIR_OVERRIDE" conda env create -n "$QIIME2_ENV_NAME" --file "$YML_FILE"
            # Pin the env to osx-64 so later installs into it also use Rosetta builds
            conda env config vars set CONDA_SUBDIR="$CONDA_SUBDIR_OVERRIDE" -n "$QIIME2_ENV_NAME" 2>/dev/null || true
        else
