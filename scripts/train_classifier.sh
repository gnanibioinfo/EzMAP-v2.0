...
# Default to all available CPU cores: this speeds up the read-EXTRACTION step
# (step 1, which is parallel). The Naive Bayes FIT (step 2) is single-threaded
# in QIIME2 and is NOT affected by this. A user-supplied --threads still wins.
...
if [[ -z "${CONDA_PREFIX:-}" ]] || [[ "${CONDA_DEFAULT_ENV:-}" != "$ENV_NAME" ]]; then
    ...
    # conda activation scripts reference unset variables (fatal under `set -u`,
    # e.g. activate-gfortran_osx-64.sh on macOS); disable nounset around it.
    set +u
...
log "  NOTE: this step is SINGLE-THREADED — extra CPU cores and RAM do not speed"
log "  it up. For a full database like SILVA it commonly takes 1–3 hours (longer"
log "  for V3–V4). It may look idle but is still working; please leave it running."
log "  Tip: a pre-trained classifier (Section 2) avoids this step entirely."
