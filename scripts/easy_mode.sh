...
CLASSIFY_JOBS=""        # empty -> falls back to $THREADS (sklearn classify workers)
CLASSIFY_BATCH=""       # empty -> QIIME2 default; set in low-memory mode to cap RAM
...
# Auto-enable low-memory mode on machines with limited RAM (<16 GB). Multi-threaded
# DADA2 and classifier loading can otherwise exhaust memory and be killed (OOM).
if ! $LOW_MEMORY; then
    if [[ "$(uname -s)" == "Darwin" ]]; then
        TOTAL_RAM_GB=$(( $(sysctl -n hw.memsize 2>/dev/null || echo 0) / 1073741824 ))
    else
        TOTAL_RAM_GB=$(( ($(getconf _PHYS_PAGES 2>/dev/null || echo 0) * $(getconf PAGE_SIZE 2>/dev/null || echo 0)) / 1073741824 ))
    fi
    if [[ "${TOTAL_RAM_GB:-0}" -gt 0 && "${TOTAL_RAM_GB}" -lt 16 ]]; then
        warn "Detected ${TOTAL_RAM_GB} GB RAM (< 16 GB) — enabling low-memory mode automatically"
        warn "  (single-threaded DADA2 + memory-capped taxonomy) to avoid out-of-memory errors."
        LOW_MEMORY=true
    fi
fi
...
if $LOW_MEMORY; then
    DADA2_THREADS=1
    # classify-sklearn loads a FULL copy of the classifier per job, so n-jobs>1
    # multiplies memory and is the #1 cause of OOM (exit 15) at the taxonomy step.
    CLASSIFY_JOBS=1
    CLASSIFY_BATCH=1000
fi
# Default DADA2 threads to the global thread count if not explicitly set
[[ -z "$DADA2_THREADS" ]] && DADA2_THREADS="$THREADS"
[[ -z "$CLASSIFY_JOBS" ]] && CLASSIFY_JOBS="$THREADS"
...
if [[ -z "${CONDA_PREFIX:-}" ]] || [[ "${CONDA_DEFAULT_ENV:-}" != "$ENV_NAME" ]]; then
    ...
    # conda's activation scripts (e.g. activate-gfortran_osx-64.sh on macOS)
    # reference unset variables, which are FATAL under `set -u`. Disable nounset
    # for the duration of activation, then restore it.
    set +u
...
if ! skipped "taxonomy"; then
    if step_needs_run "$TAXONOMY_QZA"; then
        ...
        qiime feature-classifier classify-sklearn \
            ...
            ${CLASSIFY_BATCH:+--p-reads-per-batch "$CLASSIFY_BATCH"} \
            --p-confidence 0.7 \
