...
cat("================================================================\n")
cat(" EzMAP2 — First-launch setup: installing required R packages\n")
cat("----------------------------------------------------------------\n")
cat(" This runs ONCE. EzMAP2 is installing every R package needed by\n")
cat(" the downstream analyses (relative abundance, rarefaction, alpha\n")
cat(" & beta diversity, DESeq2, ANCOM-BC, LEfSe, Random Forest,\n")
cat(" co-occurrence networks, Tax4Fun, BugBase, FunGuild).\n")
cat(" This one-time setup can take 10-30 minutes (it downloads ~40\n")
cat(" R packages). Please leave it running — afterwards EzMAP2\n")
cat(" remembers the setup and every later launch starts instantly.\n")
cat("================================================================\n")

# ============================================================================
# FAST PATH: Skip if packages were already installed successfully before.
# A lock file records the R version + the package-set version. If BOTH match,
# we skip. Bump PKGSET_VERSION whenever packages are added/removed so existing
# users automatically re-check and pick up newly-required packages (otherwise
# a stale lock would skip the install and the new packages would never arrive).
# Delete the lock file to force a re-check (e.g. after upgrading R).
# ============================================================================
PKGSET_VERSION <- "2"   # bumped: added car, FSA, rstatix, ggsignif, gtools, pROC, SpiecEasi
lock_file <- file.path(Sys.getenv("HOME"), ".ezmap2_packages_ok")
r_ver_tag <- paste0("R-", getRversion(), "_pkgset-", PKGSET_VERSION)
...
# ============================================================================
# Writable library: when R is installed under C:\Program Files\R\... (the usual
# Windows location), its main library is read-only without admin rights, so
# install.packages() fails with "'lib = ...' is not writable". We install into
# the per-user library instead, creating it if needed. R automatically adds this
# location to .libPaths() in every later session (including the Shiny launch)
# once the directory exists, so packages installed here are found at runtime.
# ============================================================================
ensure_user_library <- function() {
  user_lib <- Sys.getenv("R_LIBS_USER")
  if (nzchar(user_lib)) {
    # R_LIBS_USER may list several paths; use the first.
  ...
  # Only switch if the personal library is actually writable.
  if (dir.exists(user_lib) && file.access(user_lib, 2) == 0) {
    .libPaths(c(user_lib, .libPaths()))
    cat(sprintf("[SETUP] Installing into per-user library: %s\n", user_lib))
  } else {
    cat("[WARN] Could not create a writable user library; using default .libPaths().\n")
  }
}
ensure_user_library()
...
cran_packages <- c(
  ...
  "ggrepel",
  # ---- Statistical / plotting helpers required by specific panels ----
  # These were previously missing from the installer. Each panel guards on
  # the package with requireNamespace() and SILENTLY disables the feature
  # when it is absent, so installing them up-front is what makes every
  # analysis fully work on first launch.
  "car",       # Alpha diversity  — Levene's test (homogeneity of variance)
  "FSA",       # Alpha diversity  — Dunn's post-hoc test
  "rstatix",   # Alpha diversity  — Games-Howell post-hoc test
  "ggsignif",  # Alpha diversity  — significance brackets on boxplots
  "gtools",    # Beta diversity   — natural ordering of group levels (mixedsort)
  "pROC"       # Random Forest    — ROC / AUC performance metrics
)
...
if (!is_installed("pairwiseAdonis")) {
  ...
  tryCatch({
    ...
    if (is_installed("pairwiseAdonis")) cat("  [OK] pairwiseAdonis installed.\n")
    else cat("  [SKIP] pairwiseAdonis could not be installed (optional).\n")
  }, error = function(e) {
...
if (!is_installed("FUNGuildR")) {
  ...
  tryCatch({
    ...
    if (is_installed("FUNGuildR")) cat("  [OK] FUNGuildR installed.\n")
    else cat("  [SKIP] FUNGuildR not available for this R version (optional — fungal guild prediction disabled).\n")
  }, error = function(e) {
...
if (!is_installed("themetagenomics")) {
  ...
  tryCatch({
    remotes::install_github("EESI/themetagenomics",
                            upgrade = "never", quiet = TRUE)
    if (is_installed("themetagenomics")) cat("  [OK] themetagenomics installed.\n")
    else cat("  [SKIP] themetagenomics could not be installed (optional — Tax4Fun panel will auto-install on first use).\n")
  }, error = function(e) {
...
# ============================================================================
# Step 6: SpiecEasi (GitHub — optional, for SPIEC-EASI network inference)
# ============================================================================
# Used by the Network panel for compositionally-robust co-occurrence networks.
# Heavier than the CRAN packages (compiles huge/pulsar), so it is non-fatal:
# the Network panel falls back to Pearson/Spearman correlation if it is absent.
if (!is_installed("SpiecEasi")) {
  cat("[INSTALL] SpiecEasi (from GitHub, for SPIEC-EASI co-occurrence networks)...\n")
  ...
  tryCatch({
    remotes::install_github("zdk123/SpiecEasi", upgrade = "never", quiet = TRUE)
    if (is_installed("SpiecEasi")) cat("  [OK] SpiecEasi installed.\n")
    else cat("  [SKIP] SpiecEasi could not be installed (optional — Network panel falls back to Pearson/Spearman).\n")
  }, error = function(e) {
    cat(sprintf("  [SKIP] SpiecEasi: %s (optional — Network panel falls back to Pearson/Spearman)\n",
                conditionMessage(e)))
  })
} else {
  cat("[OK] SpiecEasi already installed.\n")
}
...
all_optional <- c(tidyverse_pkg, optional_bioc, special_packages,
                  "FUNGuildR", "themetagenomics", "SpiecEasi")
